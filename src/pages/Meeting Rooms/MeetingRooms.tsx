...
const MeetingRooms = () => {
  ...
  return (
    <div>
      {/* Search Bar */}
      <div className="flex flex-row-reverse gap-5 md:gap-16 items-center px-1 py-5 md:p-5">
        <div className="flex-1">
          <input
            ...
            className="mt-1 block w-full px-3 py-1 md:py-2 border border-gray-300 rounded-md shadow-sm focus:ring-priColor focus:border-priColor focus:outline-none  text-sm md:text-base"
            placeholder="Search..."
          />
        </div>
        <FilterForm
      ...
      {isLoading && isFetching ? (
        ...
        <>
          {data?.length === 0 ? (
            <div>
              <h3 className="text-center text-black text-opacity-60 font-bold text-xl lg:text-2xl mt-10 min-h-[60vh]">
                There is no room available
              </h3>
            </div>
          ) : (
            <div className="p-2 md:p-5 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4 md:gap-5 min-h-[60vh]">
              {data?.map((room) => (
